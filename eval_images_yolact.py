...
class detRes:
    ...
    def format(self):
        ...
        self.box = np.array(self.box, dtype=float).tolist()
        self.seg = np.array(self.seg, dtype=float).tolist()
...
def parse_args(argv=None):
    ...
    parser.add_argument('--json_path',default='/home/dwb/Pictures/zl_test/out',type=str,
...
def prep_display(dets_out, img, picName, path_, out_dir, h, w, undo_transform=True, class_color=False, mask_alpha=0.45,
                 fps_str=''):
    """
    ...
    # added by pilot
    # dets = [detRes()] * num_dets_to_consider
    for i in range(num_dets_to_consider):
        dets.append(detRes())

    print('detections count: ' + str(num_dets_to_consider))
        ...
        if on_gpu is not None and color_idx in color_cache[on_gpu]:
            ...
            if not undo_transform:
                ...
                color = (random.randint(0, 255), random.randint(0, 255),
                         random.randint(0, 255), 100)  # (7, 0, 255) #(color[2], color[1], color[0]) # (255, 0, 7)
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        for i in range(num_dets_to_consider):
            ...
            # cv2.drawContours(img_numpy, [cnt], -1, 255, cv2.FILLED)
    ...
    if args.display_text or args.display_bboxes:
        for j in reversed(range(num_dets_to_consider)):
            ...
            # m_box=[x1, y1, x2, y2]
            # m_score=score
            dets[j].score = scores[j]
            dets[j].box = [x1, y1, x2, y2]
            ...
            if args.display_text:
                ...
                # cv2.drawContours(img_numpy,[dets[j].seg],-1,(255,0,0,10),cv2.FILLED)
                zeros = np.zeros((img_numpy.shape), dtype=np.uint8)
                maskkk = cv2.fillPoly(zeros, [dets[j].seg], color)
                img_numpy = 0.95 * maskkk + img_numpy
    ...
    jobj = []
    for a in dets:
        dict = {'index': a.index, 'class': a.classs, 'score': a.score, 'box': a.box, 'seg': a.seg}
        jobj.append(dict)


    # pathh = '/media/dwb/media/cloned/yolact/dataset/zhouliu_xunlian/test/out'
    pathh=args.json_path
    jpath = pathh + '/' + picName + '.json'
    print('save json :'+jpath)

    with open(jpath, "w") as f:
        jstr = json.dump(jobj, f)
...
def evalimage(net: Yolact, path: str, save_path: str = None):
    # split to get filename,add by pilotphil
    tem_res = path.split('/')
    ress = tem_res[-1]
    picName = ress
    print('evaluating img: '+picName)
    ...
    img_numpy = prep_display(preds, frame, picName, path, out_dir, None, None, undo_transform=False)
    ...
    save_path = save_path + '/' + picName + '.jpg'
